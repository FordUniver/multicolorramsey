...
def pY {V : Type} [Fintype V] [DecidableEq V] (X Y : Finset V) [nenX: Nonempty X] (χ : TopEdgeLabelling V (Fin r))
    (i : Fin r) : ℕ :=
...
--     0 < (pY X Y χ i) := by
--   unfold pY mymin; refine (lt_min'_iff (image (fun x ↦ (N χ i x ∩ Y).card) X) _).mpr ?_
--   intros c cc
...
attribute [instance] Saga.nenX

def Saga.pY (ki : Saga χ) (i : Fin r) : ℕ :=
  min' (image (fun x => ((N χ i x) ∩ (ki.Y i)).card) ki.X) (image_nonempty.mpr (nonempty_coe_sort.mp ki.nenX))

noncomputable def Saga.p (ki : Saga χ)
    (i : Fin r) : ℝ := (ki.pY i) / ((ki.Y i).card : ℝ)
...
lemma pk_subset {X' : Finset V} (ki : Saga χ) [Nonempty X'] (sub : X' ⊆ ki.X) (i : Fin r) :
    (ki.p i) ≤ ((⟨X', ki.Y⟩ : Saga χ).p i) := by
  unfold Saga.p Saga.pY
  simp; gcongr; simp
  intros a ax
  refine Finset.min'_le _ #(N χ i a ∩ ki.Y i) ?_
  exact mem_image_of_mem _ (sub ax)
...
lemma key [Nonempty (Fin r)] -- {cardV : Fintype.card V = n}
  ...
  let N' (i : Fin r) (x : X) : (Finset V) := Classical.choose (exists_subset_card_eq (p' i x))
  ...
  have Y'lee :
    ...
    have hm (i : Fin r) :=
     ...
     _ = (f i) x / ↑(pY X (Y i) χ i) := (mul_div_cancel_right₀ _ <| by rw [← @N'card i x]; exact ne_of_gt (pos i)).symm
     _ = (f i) x / #(N' i x)  := by simp [N'card]
     _ ≤ (mymin (g i) lX') / #(N' i x) := (div_le_div_iff_of_pos_right <| pos i).mpr (mins i)
...
lemma keyk [Nonempty (Fin r)] -- {cardV : Fintype.card V = n}
  ...
  -- have :  Nonempty { x // x ∈ ki.X } := ki.nenX
  intros β C

  let p' (i : Fin r) (x : ki.X) : (ki.pY i) ≤ #(N χ i x ∩ ki.Y i) := min_le_mem _
  ...
  have Y'card {i : Fin r} : #(Y' i) = (ki.p i) * #(ki.Y i) := by
    ...
    · simp [Saga.pY, card_eq_zero.mp h]
      exact (min_const (fun x a ↦ rfl)).symm
