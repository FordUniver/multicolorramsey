...
% [DEFINITION] Hyperbolic cosine of square root
% Source: blueprint/paper/sections/section_the_proof_of_the_key_lemma.tex line 39-40 (cosh√x Taylor expansion)
% Deprecated blueprint: deprecated/02_Preliminaries.tex line 76-89 (\begin{definition} \label{def:coshsqrt})
% Lean status: ✓ Implemented as `coshSqrt` in Basic.lean
% Label: def:coshsqrt
% Dependencies: None (elementary functions)
% TODO: Add definition cosh√x = ∑_{n=0}^∞ x^n/(2n)! via Taylor expansion

\begin{definition}[Hyperbolic cosine of square root]
  \label{def:coshsqrt}
  % Source paper defines cosh√x = ∑_{n=0}^∞ x^n/(2n)!
\end{definition}
