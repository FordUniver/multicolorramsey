...
\begin{lemma}[Expectation of \(f\) is non-negative]
  \label{lem:expPos}
  \paperref{Equation (6) in proof of Lemma 3.1}
  \uses{lem:moments, def:f, lem:taylor-nonneg}
  \lean{expPos}
  %
  Let \(r, n\in\N\).
  Let \(U\) and \(U'\) be i.i.d.~random variables taking values in a finite set~\(X\), and let \(\sigma_1,\ldots,\sigma_r \colon X \to \R^n\) be arbitrary functions.
  For each \(i \in [r]\), define \(\tau_i(U,U') = \big\langle \sigma_i(U),\sigma_i(U') \big\rangle\) and \(Z_i = 3r \cdot \tau_i(U,U')\).
  Then
  %
...
%
\begin{proof}
  By \autoref{lem:taylor-nonneg}, the function \(f\) has a Taylor expansion
  %
  \begin{equation*}
    f(x_1,\dots,x_r) = \sum_{j=1}^r \sum_{\alpha \in \N^r : \alpha_j \ge 1} c_{j,\alpha} \, x_1^{\alpha_1} \cdots x_r^{\alpha_r}
  \end{equation*}
  %
  where all coefficients \(c_{j,\alpha} \ge 0\) are non-negative.

  Substituting \(Z_i = 3r \cdot \tau_i(U,U')\) where \(\tau_i(U,U') = \big\langle \sigma_i(U),\sigma_i(U') \big\rangle\), we have
  %
  \begin{align*}
    f(Z_1,\dots,Z_r) &= \sum_{j=1}^r \sum_{\substack{\alpha \in \N^r \\ \alpha_j \ge 1}} c_{j,\alpha} \, (3r)^{|\alpha|} \, \tau_1(U,U')^{\alpha_1} \cdots \tau_r(U,U')^{\alpha_r}\\
    &= \sum_{j=1}^r \sum_{\substack{\alpha \in \N^r \\ \alpha_j \ge 1}} c_{j,\alpha} \, (3r)^{|\alpha|} \, \big\langle \sigma_1(U),\sigma_1(U') \big\rangle^{\alpha_1} \cdots \big\langle \sigma_r(U),\sigma_r(U') \big\rangle^{\alpha_r},
  \end{align*}
  %
  ...
  %
  \begin{equation*}
    \Ex\big[ f(Z_1,\dots,Z_r) \big] = \sum_{j=1}^r \sum_{\substack{\alpha \in \N^r \\ \alpha_j \ge 1}} c_{j,\alpha} \, (3r)^{|\alpha|} \, \Ex\Big[ \big\langle \sigma_1(U),\sigma_1(U') \big\rangle^{\alpha_1} \cdots \big\langle \sigma_r(U),\sigma_r(U') \big\rangle^{\alpha_r} \Big].
  \end{equation*}
  %
  For each multi-index \(\alpha = (\alpha_1,\dots,\alpha_r)\), we apply \autoref{lem:moments} with the exponents \(\ell_1 = \alpha_1, \dots, \ell_r = \alpha_r\).
  This gives
  %
  \begin{equation*}
    \Ex\Big[ \big\langle \sigma_1(U),\sigma_1(U') \big\rangle^{\alpha_1} \cdots \big\langle \sigma_r(U),\sigma_r(U') \big\rangle^{\alpha_r} \Big] \ge 0.
  \end{equation*}
  %
  Since each term in the sum is a product of three non-negative quantities:
  %
  \begin{itemize}
    \item \(c_{j,\alpha} \ge 0\) (from \autoref{lem:taylor-nonneg}),
    \item \((3r)^{|\alpha|} > 0\) (positive constant),
    \item \(\Ex[\cdots] \ge 0\) (from \autoref{lem:moments}),
  \end{itemize}
  %
  we conclude that each term is non-negative, and therefore their sum is non-negative:
  %
  \begin{equation*}
    \Ex\big[ f(Z_1,\dots,Z_r) \big] \ge 0.
  \end{equation*}
  %
\end{proof}

\begin{lemma}[Expectation inequality from special function bounds]
  \label{lem:exp-ineq}
  \paperref{Equations in proof of Lemma 3.1}
  \uses{lem:expPos, lem:special:function:1, lem:special:function:2}
  \lean{exp_ineq_ENN}
  \leanok
  %
  Let \(r, n\in\N\) with \(r > 0\).
  Let \(U\) and \(U'\) be i.i.d.~random variables taking values in a finite set~\(X\), and let \(\sigma_1,\ldots,\sigma_r \colon X \to \R^n\) be arbitrary functions.
  For each \(i \in [r]\), define \(Z_i = 3r\big\langle \sigma_i(U),\sigma_i(U') \big\rangle\).
  Let \(E\) be the event that \(Z_i \ge -3r\) for every \(i \in [r]\).
  Then
  %
  \begin{equation*}
...
%
\begin{proof}
  By \autoref{lem:expPos}, we have \(\Ex\big[ f\big( Z_1,\ldots,Z_r \big) \big] \ge 0\).
  Splitting the expectation over \(E\) and \(E^c\), we have
  %
  \begin{align*}
  ...
  \end{align*}
  %
  where we use \autoref{lem:special:function:1} to bound \(f\) on \(E\), and \autoref{lem:special:function:2} on \(E^c\).
  Rearranging gives the result.
\end{proof}

\begin{lemma}[Integral bound via exponential decay]
  \label{lem:lintegral-sum-bound}
  \paperref{Calculation in proof of Lemma 3.1}
  \uses{lem:integral-exp-decay}
  \lean{lintegral_sum_bound}
  \leanok
  %
  Let \(r, n\in\N\). Set \(\beta = 3^{-4r}\) and \(C = 4r^{3/2}\).
  Let \(U\) and \(U'\) be i.i.d.~random variables taking values in a finite set~\(X\), and let \(\sigma_1,\ldots,\sigma_r \colon X \to \R^n\) be arbitrary functions.
  Define \(\tau_i(U,U') = \big\langle \sigma_i(U),\sigma_i(U') \big\rangle\) and \(Z_i = 3r \cdot \tau_i(U,U')\).
  Let \(E\) be the event that \(Z_i \ge -3r\) for every \(i \in [r]\).
  Let \(M = \max \big\{ \big\langle \sigma_i(U),\sigma_i(U') \big\rangle : i \in [r] \big\}\) and set \(c = r\sqrt{3r}\).
  %
  Assume that \(\Pr(E) < \beta\) and that for all \(\lambda \ge -1\),
  %
  \begin{equation*}
    \Pr\Big( \big\{ M \ge \lambda \big\} \cap E \Big) < \beta r e^{-C\sqrt{\lambda + 1}}.
  \end{equation*}
  %
  Then
  %
  \begin{equation*}
    \Ex\Big[ \exp\bigg( \sum_{i = 1}^r \sqrt{ Z_i + 3r } \bigg) \mathbf{1}_E \Big] \le \beta (cr + 1).
  \end{equation*}
  %
\end{lemma}
%
\begin{proof}
  For any constant \(c \le C - 1\), we have by the fundamental theorem of calculus that
  %
  \begin{equation*}
    \exp(c\sqrt{M+1})\mathbf{1}_E = \mathbf{1}_E + \int_{-1}^{\infty} \mathbf{1}_{E \cap \{M\geq \lambda\}}\frac{ce^{c\sqrt{\lambda+1}}}{2\sqrt{\lambda+1}} \, \mathrm{d}\lambda.
  \end{equation*}
  %
  Taking expectations and applying Tonelli's theorem:
  %
  \begin{align*}
    ...
    & < \, \beta +  \int_{-1}^\infty \frac{\beta r c}{2\sqrt{\lambda + 1}} \cdot e^{- C\sqrt{\lambda + 1}+c\sqrt{\lambda+1}} \,\mathrm{d}\lambda\\
    & \le \, \beta + \beta r \int_{-1}^\infty \frac{c}{2\sqrt{\lambda + 1}} \cdot e^{- \sqrt{\lambda + 1}} \,\mathrm{d}\lambda\\
    & = \, \beta (cr + 1),
  \end{align*}
  %
  where in the second inequality we used the assumption and \(\Pr(E) < \beta\); in the third we used \(c \le C - 1\); and in the last equality we used \autoref{lem:integral-exp-decay}.
  %
  The result follows by noting that \(\sum_{i=1}^r \sqrt{Z_i + 3r} \le r \sqrt{3r} \sqrt{M+1} = c\sqrt{M+1}\).
\end{proof}
...
\begin{lemma}[Geometric lemma]
  ...
  \paperref{Lemma 3.1}
  \uses{lem:exp-ineq, lem:lintegral-sum-bound}
  \lean{geometric}
  %
  Let \(r, n\in\N\). Set \(\beta = 3^{-4r}\) and \(C = 4r^{3/2}\).
...
\begin{proof}
  \uses{def:p, lem:exp-ineq, lem:lintegral-sum-bound}
  ...
  By \autoref{lem:exp-ineq}, we have
  %
  ...
  Let \(M = \max \big\{ \big\langle \sigma_i(U),\sigma_i(U') \big\rangle : i \in [r] \big\}\) and set \(c = r\sqrt{3r}\).
  By \autoref{lem:lintegral-sum-bound}, together with the assumption~\eqref{eq:max:big:and:E:no} and \(\Pr(E) \le \beta\), we have
  %
  \begin{equation*}
    \Ex\bigg[ \exp\bigg( \sum_{i = 1}^r \sqrt{ Z_i + 3r } \bigg) \mathbf{1}_E \bigg] \le \beta (cr + 1).
  \end{equation*}
  %
  Recalling that \(C = 4r^{3/2} \ge c + 1\) and \(\beta = 3^{-4r}\), it follows that
  %
  \begin{align*}
    1 - \Pr(E) &\le 3^r r \cdot \beta (r^{2}\sqrt{3r} + 1) \le 1/3,
  \end{align*}
  %
