...
\begin{proof}
  ...
  %
  We now verify a key equivalence.
  For any \(x,y\in X\), we claim that
  %
  \begin{equation*}
  ...
  \end{equation*}
  %
  (In the proof below, we will only use the forward direction.)
  ...
  \end{multline}
  %
  where the last equality follows since \(|N'_i(x)|= |N'_i(y)|= p_i|Y_i|\) and both sets are subsets of \(Y_i\).
  %
  Thus, for any \(\lambda \in \R\), we have
  %
  \begin{align*}
    \lambda \le \langle \sigma_i(x),\sigma_i(y) \rangle
    &\iff \lambda \alpha_ip_i|Y_i| \le \alpha_ip_i|Y_i| \langle \sigma_i(x),\sigma_i(y) \rangle \\
    &\iff \lambda \alpha_ip_i|Y_i| \le |N'_i(x) \cap N'_i(y)|-p_i^2|Y_i| \\
    &\iff p_i^2|Y_i| + \lambda \alpha_ip_i|Y_i| \le |N'_i(x) \cap N'_i(y)| \\
    &\iff \big( p_i + \lambda\alpha_i \big) p_i |Y_i| \le |N'_i(x) \cap N'_i(y)|,
  \end{align*}
  %
  where the first equivalence uses the positivity \(\alpha_ip_i|Y_i| > 0\), and the last follows by factoring.
