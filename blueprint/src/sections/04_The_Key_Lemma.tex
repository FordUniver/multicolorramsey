...
% Lean status: ✗ Not implemented
% Label: lem:pi:min
% Dependencies: def:boost-parameters, lem:pi:lower:bound
% TODO: Add bounds min p_i(s) ≥ p_0/2 and α_i(s) ≥ δ/2t

\begin{lemma}[Minimum bounds on p_i and α_i]
  \label{lem:pi:min}
  \uses{def:boost-parameters, lem:pi:lower:bound}
  % Source paper uses same label
\end{lemma}
...
\begin{algorithm}[Select and boost algorithm]
  \label{alg:select-boost}
...
\begin{algorithmic}[Key lemma application step]
  \label{alg:key-lemma-step}
...
\begin{algorithmic}[Decision rule for steps]
  \label{alg:step-decision}
...
\begin{algorithmic}[Termination conditions]
  \label{alg:termination}
