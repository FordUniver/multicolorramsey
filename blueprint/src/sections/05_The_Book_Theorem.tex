...
\begin{algorithm}[Multicolour book algorithm]
  \label{alg:multicolour-book}
...
\begin{theorem}[Book theorem]
  \label{thm:book}
  \uses{def:book, def:edge-colouring, def:colour-neighborhood}
  % Source paper uses same label
\end{theorem}
...
\begin{proof}[Proof of book theorem]
  \uses{thm:book, alg:multicolour-book, lem:X:lower:bound, lem:Y:lower:bound}
  % Source paper proves theorem using algorithm analysis
\end{proof}